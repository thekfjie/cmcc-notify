...
新建应用默认开启“携带发送时间”，发送内容末尾会附加
`发送时间：HH:MM:SS`。可以在应用编辑窗口关闭；时间使用服务进程的本地时区。
容器默认使用 `Asia/Shanghai`，可通过 `TZ` 环境变量覆盖。
...
`caption` 是可选的随附文字。为确保说明文字稳定显示，CMCC Notify 会先发送
`caption`，再发送一条独立的媒体消息；未填写 `caption` 时只发送媒体消息。
