...
API、多个 CMCC 账户、收件人群组和 Docker 部署。WebUI 主要用于管理和联通
...
| 概念 | 作用 |
| --- | --- |
| CMCC 账户 | 一条中国移动新消息连接，包含 API Key、启用状态、默认接收号码及可选协议地址。 |
| 默认接收号码 | 发送单人文字或多媒体消息时，如果没有临时填写号码，就使用账户的默认号码。 |
| 收件人群组 | Standalone Server 本地保存的一组手机号码，用于将同一条文字或多媒体消息逐个发送给多个号码。 |
| 通知应用 | 供监控、脚本或业务系统调用的安全入口。每个应用有独立 Token，并绑定账户、固定号码或群组及请求频率上限。 |

收件人群组不是中国移动侧的聊天群，也不会让群组成员互相看到。服务收到一次群组发送请求后，会依次为群组中的每个号码发送一条独立消息，并返回逐个号码的网关写入结果。上传文件只上传一次，随后复用 CMCC 返回的媒体 URL 逐人发送。
...
- 文字和多媒体都必须解析出明确收件号码；单人发送可使用账户的默认号码；
- `to` 与 `group` 不能同时填写；群组是本地逐号码扇出，不是中国移动侧的群聊、主题或官方群组接口；
...
- 是否允许向任意未开通、未授权或非中国移动号码投递，公开指引没有明确说明，部署者应控制收件人和发送频率。
...
- 账户名称：本服务内部使用的唯一名称，例如 `primary`。
- CMCC API Key：以 `ak_` 或 `app_` 开头。启用账户时必须填写。
- 默认接收号码：可选；用于单人文字和多媒体发送时的默认值。
- WebSocket 地址：可选；留空使用 SDK 的官方默认地址。
- 上传 API 地址：可选；留空使用 SDK 的官方默认地址。
- 启用账户：启用后会立即连接 CMCC，无需重启服务。

保存后，状态显示“已连接”才表示 WebSocket 认证和连接成功。显示“未连接”时仍可尝试发送，服务会先重新连接。

### 编辑账户

点击账户卡片中的“编辑”。账户名称可以修改；重命名时，所有引用旧名称的通知应用会同步迁移到新名称，不需要逐个修改。API Key 输入框留空表示保留现有密钥；输入新值表示替换。保存后相关连接会自动重建。

### 删除账户

点击“删除”并确认。删除账户不会删除其他账户和收件人群组，但之后不能再选择该账户发送。

完整 API Key 不会通过状态或设置 API 回传给浏览器，只显示摘要。

## 4. 通知应用（生产接入推荐）

“发送消息”页面适合人工验证连接、号码、群组和多媒体链路。正式接入监控、
...
只有显式开启“允许调用方覆盖收件人”的应用，才能在请求中传入二者之一。
...
- 群组名称必须唯一，例如 `family`、`team`。
- 每行填写一个手机号码。
- 也可以使用英文/中文逗号或分号分隔。
- 空白项会忽略，重复号码会自动去重。
- 群组名称创建后不能直接重命名；需要新建群组并删除旧群组。
...
在群组卡片中可以查看完整成员号码、修改成员或删除群组。删除群组只会删除本地号码列表，不会影响 CMCC 账户，也不会在中国移动侧执行任何群聊操作。
...
文字消息会逐个调用 `SendText`；多媒体会先上传一次，再逐个调用带不同 `to` 的 `SendMedia`。所以一个群组有 10 个号码，就会产生 10 次独立网关写入。某一个号码失败不会阻止后续号码继续处理。
...
只要至少一个号码写入成功，接口返回 HTTP `202 Accepted`；全部失败时返回 `502 Bad Gateway`。`accepted_count` 表示成功写入网关连接的数量，不是最终送达数量。
...
多媒体使用与文字相同的明确收件人路由。2026-09-16 已在真实中国移动新消息终端验证 PNG 图片与 ZIP 文件可以按 `to` 手机号送达。其他图片、音频、视频和文件格式仍取决于 CMCC 网关和接收终端支持情况。
...
1. 选择发送账户；
2. 选择“单个号码”并填写号码，或选择一个本地收件人群组；
3. 媒体类型保持“自动识别”，或手动选择图片、音频、视频、文件；
4. 选择最大 200 MiB 的本地文件；
5. 可填写附言并提交。

服务会先将文件上传到 CMCC `/upload` 接口，再把返回的媒体 URL、收件号码、文件名、大小、MIME 类型和媒体类型写入 WebSocket 消息。群组发送复用同一个媒体 URL。
...
| --- | --- | --- |
| `GET` | `/healthz` | 无认证健康检查。 |
| `GET` | `/v1/status` | 服务版本、运行时间和账户连接状态。 |
| `GET` | `/v1/settings` | 获取可管理的账户摘要与完整群组成员。 |
| `POST` | `/v1/accounts` | 添加账户。 |
| `PUT` | `/v1/accounts/{name}` | 编辑或重命名账户；空 `api_key` 保留原密钥，重命名会同步通知应用引用。 |
| `DELETE` | `/v1/accounts/{name}` | 删除账户。 |
| `POST` | `/v1/groups` | 创建群组。 |
| `PUT` | `/v1/groups/{name}` | 更新群组成员。 |
| `DELETE` | `/v1/groups/{name}` | 删除群组。 |
...
| `POST` | `/v1/send` | 发送单人/群组文字或远程 URL 多媒体。 |
| `POST` | `/v1/send/media` | 上传本地文件并发送给单个号码或群组。 |
