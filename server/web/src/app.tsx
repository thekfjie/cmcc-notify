...
type ApplicationSetting = {
  name: string; enabled: boolean; account?: string; group?: string; token_hint: string; rate_limit_per_minute: number; include_send_time: boolean
}
...
function ApplicationEditor({ token, accounts, groups, existing, onClose, onSaved }: { token: string; accounts: AccountSetting[]; groups: ChannelGroup[]; existing: ApplicationSetting | null; onClose: () => void; onSaved: (settings: SettingsResponse, name: string) => void }) {
  ...
  const [enabled, setEnabled] = useState(existing?.enabled ?? true); const [includeSendTime, setIncludeSendTime] = useState(existing?.include_send_time ?? true); const [rateLimit, setRateLimit] = useState(existing?.rate_limit_per_minute ?? 60)
  const [saving, setSaving] = useState(false); const [error, setError] = useState("")
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError("")
    ...
    try {
      ...
      const next = await apiRequest<SettingsResponse>(path, token, { method: existing ? "PUT" : "POST", body: JSON.stringify({ name: name.trim(), enabled, account: targetMode === "channel" ? account : "", group: targetMode === "group" ? group : "", include_send_time: includeSendTime, rate_limit_per_minute: rateLimit }) })
  ...
  return <Dialog title={existing ? `编辑应用 · ${existing.name}` : "新建通知应用"} description="应用使用独立 Token 调用通知接口，不会接触管理令牌或 CMCC API Key。" onClose={onClose}><form className="editor-form" onSubmit={submit}>
    ...
    <label className="toggle-row"><span><strong>携带发送时间</strong><small>在通知正文末尾附加本地时间，例如“发送时间：18:30:05”</small></span><input type="checkbox" checked={includeSendTime} onChange={(event) => setIncludeSendTime(event.target.checked)} /></label>
...
function DocsPage({ status }: { status: StatusResponse }) {
  ...
  const examples = useMemo(() => [
    ...
    { title: "创建通知应用", method: "POST", path: "/v1/applications", auth: true, code: `curl -X POST ${baseURL}/v1/applications \\\n  -H "Authorization: Bearer <CMCC_NOTIFY_AUTH_TOKEN>" \\\n  -H "Content-Type: application/json" \\\n  -d '{"name":"monitoring","enabled":true,"account":"${exampleAccount}","include_send_time":true,"rate_limit_per_minute":60}'` },
